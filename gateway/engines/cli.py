"""The CLI engine: build the `claude` invocation, spawn it, and yield CanonicalEvents.

This is the gateway's original and default engine — a stateless subprocess call to
the local `claude` CLI, riding this machine's Claude login. It is the only engine
that can reach company data, because the MCP server is attached to the CLI.

It knows nothing about routing. ``engine.select_engine`` decides that this request
is a CLI request and hands it over; everything here is the transport.
"""
import asyncio
import json
import logging
...
VERB = "run_claude"  # the journald prefix operators grep for; see usage_log.outcome
...
async def run_claude(req: CanonicalRequest) -> AsyncIterator[CanonicalEvent]:
    ...
    try:
        ...
        usage_log.outcome(VERB, "saturated", req, sat.waited_s,
                          ...
                          total_ms=sat.queue_wait_ms, prompt_bytes=len(stdin_data),
    ...
    try:
        ...
        def _log(result, elapsed, *a, **kw):
            ...
            usage_log.outcome(VERB, result, req, elapsed, *a, **kw)
        ...
        try:
            while True:
                ...
                if remaining <= 0:
                    process.kill()
                    await process.wait()
                    _log("timeout", loop.time() - start, level=logging.WARNING)
                ...
                if otype == "stream_event":
                    ...
                    if obj.get("is_error") or obj.get("subtype") != "success":
                        ...
                        _log("overloaded" if overloaded else "error",
                             loop.time() - start, reason=msg, level=logging.WARNING)
            ...
            if not started:
                ...
                _log("no-output", loop.time() - start, reason=msg, level=logging.WARNING)
            ...
            try:
                ...
                pass
            _log("cancelled", loop.time() - start, level=logging.WARNING)
            raise
            ...
            try:
                ...
                pass
            _log("exception", loop.time() - start, reason=str(e), level=logging.ERROR)
            yield Error(500, str(e))
    finally:
        # Guaranteed slot release. The queue-wait timeout path returns before
        # entering this try, so nothing is released that was not acquired.
        lanes.release(lane)
