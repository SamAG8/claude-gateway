...
**Model Map** — `models.json` (resolved by `gateway/models.py`, hot-reloaded by mtime); resolves a client's model string to a real `claude --model` value via passthrough (`claude-*`) → alias → default. Unknown models fall back to the default rather than erroring. Operational policy is family-aware: a dated id such as `claude-haiku-4-5-20251001` inherits the `haiku` effort, thinking budget, and fast-lane settings.
...
**Usage Log** — the per-invocation accounting the engine emits on every terminal outcome. Always: one human-readable line to journald (`run_claude success surface=… model=… in=… out=… queue_ms=… first_text_ms=… total_ms=…`). Additionally, when `USAGE_LOG` is set, one structured JSON line (`gateway/usage_log.py`) carrying queue/spawn/stdin/first-event/first-text/total timings, token/cache counters, prompt byte and history-message counts, media counts, model/lane, and whether MCP was attached. It never stores prompt/response content or credentials. `scripts/usage_report.py` aggregates P50/P95/P99 by model and MCP path plus token/cost totals; `scripts/benchmark_latency.py` measures end-to-end deployed TTFT/total percentiles. Cache counters read from the CLI's `usage` object; native-vision PDFs (Gemini surface) show up as `docs`, while Anthropic-surface PDFs are already text by the time the engine sees them.
