...
**Usage Log** — the per-invocation accounting the engine emits on every terminal outcome. Always: one human-readable line to journald (`run_claude success surface=… model=… in=… out=… cache_read=… cache_write=… imgs=… docs=…`). Additionally, when `USAGE_LOG` is set, one structured JSON line (`gateway/usage_log.py`) carrying tokens, cache read/write, native image/document counts, elapsed, surface, and a **reference** cost (`gateway/pricing.py`, editable via `pricing.json`). Cost is a comparison aid only — in the default `clean` isolation the gateway spends a Claude *subscription*, not per-token API billing. `scripts/usage_report.py` aggregates the JSONL (totals, cache-hit ratio, by model/surface/outcome, priciest calls). Cache counters read from the CLI's `usage` object; native-vision PDFs (Gemini surface) show up as `docs`, while Anthropic-surface PDFs are already text by the time the engine sees them.
